    ...
    <div className="space-y-6">
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="relative overflow-hidden px-5 py-6 sm:px-7 sm:py-7">
          ...
          <div className="relative flex flex-col gap-5 sm:flex-row sm:items-start sm:justify-between">
            ...
            {canSeeAll && (
              <div className="inline-flex min-h-12 shrink-0 rounded-xl border border-slate-200 bg-slate-50 p-0.5" aria-label="Task scope">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-1.5">
            ...
            {[
              ...
              <button
                ...
                className={`min-h-11 rounded-lg border px-3 py-2 text-[11px] font-semibold transition ${
                  dueAt === option.value
            ...
            {dueAt && (
              <button type="button" onClick={() => setDueAt("")} className="min-h-11 rounded-lg px-3 py-2 text-[11px] font-semibold text-slate-500 hover:bg-slate-50 hover:text-slate-700">
                Clear date
              </button>
            )}
          </div>
      ...
      {error && (
        <div role="alert" className="flex flex-col gap-3 rounded-xl border border-red-200 bg-red-50 px-4 py-3 text-sm text-red-700 sm:flex-row sm:items-center sm:justify-between">
          <span>{error}</span>
          <button type="button" onClick={fetchTodos} className="min-h-11 w-fit rounded-lg px-2 text-xs font-semibold text-red-700 hover:bg-red-100 hover:text-red-900">
            Refresh tasks
          </button>
        </div>
      )}
          ...
          {FILTERS.map((item) => {
            ...
            return (
              <button
                ...
                className={`flex min-h-11 shrink-0 items-center justify-center gap-1.5 whitespace-nowrap rounded-xl px-3.5 text-xs font-semibold transition sm:px-2 sm:text-sm ${
                  selected
                    ? urgent
                  ...
                  {confirming && (
                    <div className="flex flex-col gap-3 border-t border-red-100 bg-red-50/70 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5">
                      ...
                      <div className="flex gap-2">
                        <button type="button" onClick={() => setConfirmingDelete(null)} className="min-h-11 rounded-lg px-3 py-2 text-xs font-semibold text-slate-600 transition hover:bg-white">
                          Keep task
                        </button>
                        <button
                          type="button"
                          ...
                          className="min-h-11 rounded-lg bg-red-600 px-3 py-2 text-xs font-semibold text-white transition hover:bg-red-700 disabled:opacity-50"
                        >
