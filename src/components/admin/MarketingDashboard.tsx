...
const ChartLoading = () => (
  <div className="h-full flex items-center justify-center text-sand-400 text-sm animate-pulse">Loading chart...</div>
);
const RevenueSpendPanels = dynamic(() => import("./MarketingCharts").then((m) => m.RevenueSpendPanels), {
  ssr: false,
  loading: () => (
    <div className="h-[228px] flex items-center justify-center text-sand-400 text-sm animate-pulse">Loading chart...</div>
  ),
});
const AdSpendChart = dynamic(() => import("./MarketingCharts").then((m) => m.AdSpendChart), { ssr: false, loading: ChartLoading });
const RoasChart = dynamic(() => import("./MarketingCharts").then((m) => m.RoasChart), { ssr: false, loading: ChartLoading });
const ConversionsChart = dynamic(() => import("./MarketingCharts").then((m) => m.ConversionsChart), { ssr: false, loading: ChartLoading });
const ClicksChart = dynamic(() => import("./MarketingCharts").then((m) => m.ClicksChart), { ssr: false, loading: ChartLoading });
const CpcChart = dynamic(() => import("./MarketingCharts").then((m) => m.CpcChart), { ssr: false, loading: ChartLoading });
const CtrChart = dynamic(() => import("./MarketingCharts").then((m) => m.CtrChart), { ssr: false, loading: ChartLoading });
const ProfitChart = dynamic(() => import("./MarketingCharts").then((m) => m.ProfitChart), { ssr: false, loading: ChartLoading });
const AovChart = dynamic(() => import("./MarketingCharts").then((m) => m.AovChart), { ssr: false, loading: ChartLoading });
const OrdersChart = dynamic(() => import("./MarketingCharts").then((m) => m.OrdersChart), { ssr: false, loading: ChartLoading });
const ImpressionsVisitsChart = dynamic(() => import("./MarketingCharts").then((m) => m.ImpressionsVisitsChart), { ssr: false, loading: ChartLoading });

// These tabs use recharts internally, so they're loaded on demand too.
const CampaignsTab = dynamic(() => import("./marketing/CampaignsTab"), {
  ssr: false,
  loading: () => <div className="text-center py-12 text-sand-400">Loading...</div>,
});
const AudienceTab = dynamic(() => import("./marketing/AudienceTab"), {
  ssr: false,
  loading: () => <div className="text-center py-12 text-sand-400">Loading...</div>,
});
...
function InsightsPanel({
  ...
  if (c.clicks > 0 && c.ad_spend > 0) {
    ...
    insights.push({
      ...
      text: `Cost per click is ${formatCAD(cpc)}. ${
  ...
  if (c.order_count > 0 && c.revenue > 0) {
    ...
    insights.push({
      icon: "#",
      color: "text-sand-700",
      text: `Average order value is ${formatCAD(aov)} across ${c.order_count} orders. ${
            ...
            <div className="grid grid-cols-1 xl:grid-cols-[1fr_340px] gap-6">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                ...
                <ChartCard title={`Ad Spend (${rangeLabel})`} chartKey="Ad Spend" avg={formatCurrency(avgs.ad_spend) + perUnit}>
                  <AdSpendChart data={chartData} weekly={isWeekly} avg={avgs.ad_spend} />
                </ChartCard>
                ...
                >
                  <RoasChart data={chartData} weekly={isWeekly} avg={avgs.roas} />
                </ChartCard>

                {/* Conversions */}
                <ChartCard title={`Conversions (${rangeLabel})`} chartKey="Conversions" avg={formatNumber(avgs.conversions) + perUnit}>
                  <ConversionsChart data={chartData} weekly={isWeekly} avg={avgs.conversions} />
                </ChartCard>

                {/* Clicks */}
                <ChartCard title={`Clicks (${rangeLabel})`} chartKey="Clicks" avg={formatNumber(avgs.clicks) + perUnit}>
                  <ClicksChart data={chartData} weekly={isWeekly} avg={avgs.clicks} />
                </ChartCard>

                {/* CPC Over Time */}
                <ChartCard title={`CPC (${rangeLabel})`} chartKey="CPC" avg={formatCAD(avgs.cpc)}>
                  <CpcChart data={chartData} weekly={isWeekly} avg={avgs.cpc} />
                </ChartCard>

                {/* CTR Over Time */}
                <ChartCard title={`CTR (${rangeLabel})`} chartKey="CTR" avg={formatPct(avgs.ctr)}>
                  <CtrChart data={chartData} weekly={isWeekly} avg={avgs.ctr} />
                </ChartCard>

                {/* Profit Over Time */}
                <ChartCard title={`Profit (${rangeLabel})`} chartKey="Profit" avg={formatCurrency(avgs.profit) + perUnit}>
                  <ProfitChart data={chartData} weekly={isWeekly} avg={avgs.profit} />
                </ChartCard>

                {/* AOV Over Time */}
                <ChartCard title={`Avg Order Value (${rangeLabel})`} chartKey="AOV" avg={formatCAD(avgs.aov)}>
                  <AovChart data={chartData} weekly={isWeekly} avg={avgs.aov} />
                </ChartCard>

                {/* Order Count Per Day */}
                <ChartCard title={`Orders Per  ()`} chartKey="Order Count" avg={formatNumber(avgs.order_count) + perUnit}>
                  <OrdersChart data={chartData} weekly={isWeekly} avg={avgs.order_count} />
                </ChartCard>
                ...
                {hasGA4 && chartData.some((d) => d.sessions !== undefined) && (
                  <ChartCard title={`Impressions vs Visits (${rangeLabel})`} chartKey="Impressions vs Visits">
                    <ImpressionsVisitsChart data={chartData} weekly={isWeekly} />
                  </ChartCard>
                )}
