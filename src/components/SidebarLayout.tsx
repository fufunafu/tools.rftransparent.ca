...
export default function SidebarLayout({ children }: { children: React.ReactNode }) {
  ...
  const foldButton = (
    ...
    >
      {/* The double chevron the library's own rail uses, so the two apps fold
          with the same gesture and the same mark. */}
      <svg
        viewBox="0 0 24 24"
        fill="none"
        stroke="currentColor"
        ...
        className={`w-[15px] h-[15px] transition-transform duration-200 ${collapsed ? "-scale-x-100" : ""}`}
      >
        <path d="M14.5 6.5L9 12l5.5 5.5" />
        <path d="M19 6.5L13.5 12 19 17.5" />
      </svg>
    </button>
  );
