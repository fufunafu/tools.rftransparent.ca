-- Refine lead_distribution to represent the follow-up PIPELINE only:
-- leads where a rep has logged at least one follow-up (logged_by != 'system').
-- Auto-wins from Shopify sync are excluded. The 'pending' bucket is dropped
-- (pending leads aren't in the pipeline by definition).
--
-- Renumbered from 024.
-- Safe to re-run.
...
as $$
  ...
  lead_distribution_agg as (
    -- Pipeline-only: include only leads with at least one human-logged follow-up.
    select coalesce(jsonb_object_agg(bucket, c), '{}'::jsonb) as lead_distribution
    from (
      select
        lead_status as bucket,
        count(*)::int as c
      from filtered f
      where exists (
        select 1 from followup_logs
        where lead_id = f.id
          and logged_by <> 'system'
      )
      group by lead_status
    ) d
  )
