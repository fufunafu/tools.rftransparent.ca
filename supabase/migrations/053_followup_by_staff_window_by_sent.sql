-- Quotes-by-Staff now windows each quote by when it was LAST sent
-- (coalesce(last_invoice_sent_at, shopify_created_at)) instead of creation date,
-- so a quote re-sent today lands in today's window under whoever sent it.
-- Attribution is still the last invoice sender (migration 051). Safe to re-run.
...
as $$
  with filtered as (
    ...
    where store_id = p_store_id
      ...
      and (p_cutoff is null
           or coalesce(last_invoice_sent_at, shopify_created_at) >= p_cutoff)
  )
  select
    f.staff,
    count(*)::int as total,
